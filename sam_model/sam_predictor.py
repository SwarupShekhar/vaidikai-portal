...
class InMemoryLRUDictCache(object):
    """A simple self-contained LRU cache to avoid external package issues."""
...
MOBILESAM_CHECKPOINT = os.environ.get("MOBILESAM_CHECKPOINT", "/data/models/mobile_sam.pt")
    ...
    def __init__(self, model_choice):
        ...
        if model_choice == 'ONNX':
            ...
            if not os.path.exists(self.model_checkpoint):
                logger.warning(f"Model not found at {self.model_checkpoint}, downloading...")
                ...
                logger.info(f"Successfully downloaded MobileSAM weight to {self.model_checkpoint}")
